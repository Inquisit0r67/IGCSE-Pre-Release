            ...
            while (EOD == false)
            {
                ...
                while (Check == true)
                {
                    ...
                    { //If its end of day the program prints out the number of orders made and all the unique order numbers
                        ...
                        for (int z = 0; z < Running_Stock.Length; z++) //Iterates through stock
                        {
                            Write(Stock[z] - Running_Stock[z]); //Subtracts Stock at start from current stock to get amount sold
                        }
                        ReadKey();
                        ReadKey();
                        EOD = true;
